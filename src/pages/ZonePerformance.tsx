...
export default function ZonePerformance() {
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <h1 className="text-2xl font-heading font-bold text-foreground">Zone Performance</h1>

      {/* Chart */}
      <div className="glass-card p-5">
        <h3 className="font-heading font-semibold text-sm mb-4 text-foreground">Tours by Zone</h3>
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              <Tooltip contentStyle={{ background: 'hsl(220 18% 12%)', border: '1px solid hsl(220 14% 16%)', borderRadius: '8px', fontSize: '12px', color: 'hsl(210 20% 92%)' }} />
              <Bar dataKey="tours" fill="hsl(217 91% 60%)" radius={[4, 4, 0, 0]} opacity={0.8} name="Scheduled" />
              <Bar dataKey="completed" fill="hsl(152 69% 45%)" radius={[4, 4, 0, 0]} opacity={0.8} name="Completed" />
              <Bar dataKey="drafts" fill="hsl(38 92% 50%)" radius={[4, 4, 0, 0]} opacity={0.8} name="Drafts" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {zonePerf.map(z => {
          ...
          return (
            <div key={z.zoneId} className="glass-card p-5">
              <h3 className="font-heading font-semibold text-foreground mb-1">{z.zoneName.split(' — ')[1]}</h3>
              <p className="text-xs text-muted-foreground mb-4">{flowCount} Flow Ops · {tcmCount} TCM</p>
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={cn('font-medium', z.showUpRate >= 70 ? 'text-role-tcm' : z.showUpRate >= 50 ? 'text-role-hr' : 'text-danger')}>
                    {z.showUpRate}%
                  </span>
                </div>
